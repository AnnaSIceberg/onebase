...
3. По состоянию:
   ...
   - **DIRTY (конфликт)** → чинить в отдельном worktree, привязанном к SHA
     ...
     **Для `Plans/README.md` разрешена перенумерация:** в добавленных строках
     ...
     `CHANGELOG.md` добавлен в список по #1497: в него дописывает каждый
     влитый PR, поэтому при base-sync давно открытого PR он конфликтует почти
     всегда. Повод — #1220, где обе стороны только дописывали свои записи, а
     мерж ship-PR из-за этого останавливался и требовал человека.
