...
1. Первичный список получай целиком пагинированным REST — жёсткого cap быть не
   ...
   Открытым для recovery считается только intent, чей `from` равен текущему
   HEAD либо первому parent текущего двухродительского HEAD. Незавершённые
   intent от прежних HEAD — только audit history: они не владеют single-flight
   и не отменяют более позднюю завершённую intent/done-цепочку. При валидном
   done текущего HEAD unmatched intent с тем же первым parent является stale
   параллельным дубликатом, а не новой транзакцией.
   ...
   **обычное полное содержательное REVIEW**, даже если осталась старая метка
   ...
   каноничной committed-пары REVIEW не ставит `ship`: требуется новый
   ...
   **Запрещено реконструировать историческую epoch** подменой текущего
   ...
   claim, ни completion в следующих запусках. Точный `pp:fix-decision <текущий
   SHA>` передаёт этот SHA только FIX: промежуточное состояние с обеими метками
   остаётся припаркованным, затем FIX делает одну доработку, публикует новый HEAD
   и завершает post-push handoff снятием `changes-requested`. Лишь после этого
   новый HEAD без committed-пары входит в REVIEW ровно один раз; первая
   committed-пара этого HEAD снова закрывает его для повторов. Отдельный
   `pp:review-again` — маршрут без доработки к одному повторному аудиту текущего
   SHA, а не разрешение FIX.

   Для каждого оставшегося PR получи текущий SHA явным JSON-полем:
   ...
   **Не удаляй её перед аудитом** — у общей метки нет владельца, и DELETE может
   ...
   proof `from`; `ship-event` — последний trusted `LabeledEvent` в эпохе
   `from` после anchor этого HEAD. Он может предшествовать completion: source
   proof всё равно обязан впоследствии стать каноничным на том же неизменном
   HEAD, а любой более поздний unlabel/чужой label отменяет разрешение.
   ...
   адресованный `ship-event` расположен до source HEAD anchor либо был отменён
   ...
   **Интеграционное REVIEW не повторяет содержательный аудит.** Валидный
   ...
   | есть валидный `pp:base-sync-v1-aborted` текущего HEAD, `ship` снят | обычное полное содержательное REVIEW; после успеха ждать нового человеческого `ship` |
...
3. Прогон (если дифф трогает Go или прикладной слой) — читать дифф глазами мало:
   ...
   Если diff затрагивает Go или прикладной слой, там `go build ./...` обязателен.
   Также обязательно выполни
   `go test -count=1` и `go vet` для затронутых пакетов и тех конкретных
   пакетов-потребителей, чьё поведение мог изменить diff. Если менялся движок
   конфигураций или примеры — обязательно выполни
   `go run ./cmd/onebase check --project examples/trade`. Зелёный CI точного
   HEAD не заменяет эти локальные проверки, а актуальный обязательный CI точного
   HEAD остаётся обязательным гейтом.
   ...
   заранее названном в заключении конкретном триггере:
   ...
   - diff/base-sync-дельта меняет сквозную инфраструктуру: `go.mod`/`go.sum`,
     toolchain/build tags, генерацию, общий test harness, глобальную
     инициализацию или общий контракт, для которого нельзя надёжно ограничить
     круг потребителей;
   - это репозиторный рефакторинг нескольких независимых подсистем, и полный
     список затронутых пакетов и потребителей нельзя обоснованно перечислить.
   ...
   Полный набор при наличии триггера разрешён, но не обязателен. Worktree убрать.
   **Что именно запускал — перечисли в заключении**, а для полного набора назови
   ещё и триггер. Слово «проверено» без прогона писать нельзя: зелёный отчёт о
   непроведённой проверке хуже отсутствующего.
   ...
   классифицируй каждую его ошибку. Зелёных целевых тестов и обязательного CI
   для вывода о шуме окружения недостаточно. Такой вывод требует положительного
   доказательства: ошибка совпадает с известной задокументированной сигнатурой
   со ссылкой на документ или issue либо точный падающий пакет/тест контрольным
   прогоном воспроизводится на неизменённом base в той же среде. Иначе локализуй
   ошибку и повтори точный падающий пакет/тест, а не весь набор; не одобряй PR,
   пока причина не классифицирована. Связанное с diff падение блокирует ревью.
...
7. Заключение — комментарий в PR. Кандидат в заключения содержит отдельную
   ...
   не мешает восстановить и завершить его review-транзакцию. При доказанном
   незавершённом base-sync сохранённый `ship`
   ...
   sticky-разрешением на последующий MERGE. Для обычного заключения
   `changes-requested` и
