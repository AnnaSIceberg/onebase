...
`gh pr diff` не имеет флага `--stat`: не используй его ни для первичного
просмотра, ни как необязательную диагностику. Полный патч получай только
`gh pr diff <M>`, а список и размеры файлов — уже запрошенным JSON-полем
`files` команды `gh pr view`. Отсутствие красивой сводки не является причиной
останавливать REVIEW.
...
9. Финал: `ИТОГ: ГОТОВО (отревьюено 2: #a — годится, #b — замечания)` /
   ...
   авторизации, JSON, API/лимита, тестов или частично начатая транзакция обязана
   сохранить настоящий `НЕ СМОГ`/`НУЖЕН ЧЕЛОВЕК`.
