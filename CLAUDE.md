...
- **Мёрж нескольких PR — по одному, не пачкой.** После каждого мержа влей свежий
  ...
  мержи следующий.
  - `docs/features.md`, `CHANGELOG.md`, `Plans/README.md` и
    `internal/i18n/locales/*.json` могут меняться в параллельных PR, поэтому
    после каждого мержа заново проверяй конфликты в оставшихся ветках.
    У дописываемых файлов MERGE берёт обе стороны
    сам, но обязан проверить целые добавленные блоки с контекстом, порядком строк
    и числом вхождений (`tools/mergecheck`, #1497). В `Plans/README.md` конфликтует
    нумерация — пункты надо перенумеровать руками; менять можно только первый
    числовой столбец, сохраняя содержание, ссылки, порядок и повторы. Уникальность
    номеров и существование ссылок проверяются отдельно.
