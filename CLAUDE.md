...
- **Сначала выбери маршрут работы по `CONTRIBUTING.md`.** Короткое сообщение о
  баге, скриншот или диагностический файл уже являются полезной обратной связью:
  полное оформление issue желательно, но не обязательно. Если автор не берёт
  реализацию, подтверждённая находка идёт в issue и конвейер. Если контрибутор
  сам проверяет гипотезу, работа начинается в fork/экспериментальной ветке и
  Draft PR; PromptPilot получает её только после перевода в Ready. Issue для
  такого Draft не обязательна. Перед началом исключи параллельный FIX/PR той же
  работы; не объединяй задним числом PR, уже вошедшие в protocol REVIEW/FIX.
- **Ветки:** `feature/*`, `fix/*`, для проверки гипотез — `experiment/*` от
  `main`; не коммить напрямую в `main`.
...
- **Конвейер сопровождения — метки, а не договорённости.** Заявки и PR ведутся
  полуавтоматически скилами `.claude/skills/` (`/triage-issues`, `/plan-approved`,
  `/fix-approved`, `/review-queue`, `/merge-shepherd`, `/tail-issues`,
  `/discussions-watch`). Разделение простое:
  **починка — не решение, новая возможность — решение.**
  ...
  к человеку). Мерж-этап разрешает `ship` на **PR**, ставит его только человек;
  автоматика MERGE без `ship` не вливает. Это гейт конвейера, а не правило
  GitHub branch protection: ручной admin-merge технически может обойти метку и
  остаётся ответственностью человека. Ограничение «не трогать» относится к MERGE,
  ...
  - **Обсуждения — вход в проект мимо заявок; этап не использует маршрутные
    метки.** Состояние задают последнее сообщение и доверенные служебные маркеры,
    поэтому единственная сверка — этап `/discussions-watch`: тред, где последнее
    слово за человеком со стороны, получает ответ, а просьба — номер обычной
    заявки, которую дальше разбирает триаж на общих основаниях.
    Каждый protocol-ответ коммитит hash точной внешней записи: сообщение,
    пришедшее между последней сверкой и POST ответа, остаётся отдельным
    кандидатом, даже если машинный комментарий создан позже. До любого ответа
    этап выбирает единственного writer через каноничный claim и 30-минутную
    lease: из одновременных roots продолжает только worker с собственным
    earliest GraphQL edge, а истёкшее владение восстанавливается takeover.
    Создание заявки
    сериализует постоянный create-only ref
    `pp-discussion-dedupe/<source-sha256>`; exact-source recovery читает прямой
    пагинированный REST, а orphan ref/intent без найденной заявки передаётся
    человеку и никогда не повторяет неидемпотентный create автоматически.
    Расписание этапов живёт вне репозитория и правкой этой строки не
    переключается; на 29.08.2026 задачи для `/discussions-watch` в планировщике
    нет — его зовут вручную. Что заведено на самом деле — сверяй у планировщика
    (`docs/maintenance-pipeline.md`, раздел «Расписание и ручной запуск»).
    Отвечать разрешено только тем, что проверено прогоном: обсуждения читают
    те, кто ещё выбирает платформу, и «так работает» без проверки обесценивает
    все остальные ответы разом. Отдельный провайдер
    `claude-discussions` пока не установлен; до его появления этап запускается
    вручную. Его будущий allowlist обязан включать используемые процедурой
    read-only REST-запросы и Create ref только в
    `refs/heads/pp-discussion-dedupe/`, а не только GraphQL. Писать в обсуждение
    может любой пользователь GitHub, это самый недоверенный текст в репозитории.
    Универсальный `gh api graphql` технически умеет менять метки, поэтому
    запрет таких мутаций задаёт процедура, а не песочница провайдера.
  - **Припаркованная заявка обязана попадать ровно в одну корзину:** «делаем
    планом» (`hold` + ссылка на план), «ждём предшественника» (`hold` + строка
    `Blocked-by: #1204`), «не делаем» (закрыта с причиной) или «не решено»
    (`needs-decision`). Сто заявок в первых двух — дорожная карта, двадцать без
    плана и без предшественника — свалка: через месяц не отличить решённое от
    забытого. Сверка — `go run ./tools/backlogsweep` раз в неделю: внешние
    заявки без ответа, `hold` без плана и без движения, зависший
    `needs-decision`, ссылки на ненаписанные планы, закрытый предшественник при
    живой паузе. Не гейт, как `issuetail` и `featureage`.
    - **Пауза, которая ждёт другую заявку, объявляется строкой** — без неё
      «ждём предшественника» и «просто лежит» выглядят одинаково, и снятие
      первой держится на памяти человека, против чего эти сверки и написаны.
      Объявленную `backlogsweep` ведёт сам: пока предшественник открыт,
      залежавшейся паузу не считает, а как только тот закрыт — показывает
      строкой «пора снимать `hold`». Подробности — в
      `docs/maintenance-pipeline.md`.
