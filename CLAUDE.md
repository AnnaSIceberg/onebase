...
- **Конвейер сопровождения — метки, а не договорённости.** Заявки и PR ведутся
  ...
  **починка — не решение, новая возможность — решение.**
  ...
  обычная процедура. До первой мутации выполняются ровно два полных
  `pipelinehealth` scan: election и свежий gate. После продуктивного completion
  PromptPilot отдельно запускает свежий scan для wake-up следующих этапов,
  поэтому весь продуктивный fallback сокращается с четырёх scan до трёх
  (`4 → 3`), а не до двух. Экономится повторная election, не свежие проверки.
