...
func parseTablePartRows(r *http.Request, entity *metadata.Entity) (map[string][]map[string]any, error) {
	return parseTablePartRowsForManagedForm(r, entity, nil, true)
}
...
func parseTablePartRowsForManagedForm(
	...
	return parseLegacyTablePartRows(r, entity)
}
...
func parseManagedTablePartRows(
	...
	for _, tp := range entity.TableParts {
		...
		case managedFormTableJSONPayload:
			...
			converted, convErr := convertManagedTablePartRows(rawRows, tp)
			if convErr != nil {
				return nil, convErr
			}
			result[tp.Name] = converted
...
// tablePartFieldValue приводит присланное браузером значение реквизита
// табличной части к типу этого реквизита — ОДИН раз для всех форм.
//
// До #1074 приведение было размножено: этот switch и его копия в
// parseLegacyTablePartRows, и обе разбирали только number и bool. Дата попадала
// в default и уезжала в базу строкой как есть. На SQLite колонка даты — TEXT, и
// «14.03.1985» (да и любой другой текст) ложился туда молча, с ответом 303: для
// пользователя это выглядело как успешное сохранение. На PostgreSQL та же
// строка давала 500. Ячейка даты при этом редактируется свободным текстом —
...
// действие — на SQLite клал в колонку даты пустую строку, а на PostgreSQL давал
// 500. Это не тот дефект, который описан в заявке, но тот же самый: значение
// уходит в базу, не будучи приведённым к типу колонки.
//
// Признак «пустая строка ТЧ» ниже считается через fmt.Sprintf("%v", value), где
...
func convertManagedTablePartRows(rows []map[string]any, tablePart metadata.TablePart) ([]map[string]any, error) {
	...
	for _, row := range rows {
		...
		for _, field := range tablePart.Fields {
			...
			value, err := tablePartFieldValue(field, raw)
			if err != nil {
				return nil, fmt.Errorf("таблица %q: %w", tablePart.Name, err)
			}
			converted[field.Name] = value
		}
		empty := true
		for _, field := range tablePart.Fields {
			if value, present := converted[field.Name]; present && value != nil && fmt.Sprintf("%v", value) != "" {
				empty = false
				break
			}
		}
...
func parseLegacyTablePartRows(r *http.Request, entity *metadata.Entity) (map[string][]map[string]any, error) {
	result := make(map[string][]map[string]any)
	for _, tp := range entity.TableParts {
		prefix := "tp." + tp.Name + "."
		hasNamedRows := false
		...
		if jsonBlob := r.FormValue("tp_json." + tp.Name); !hasNamedRows && jsonBlob != "" {
			...
			if err := json.Unmarshal([]byte(jsonBlob), &rows); err == nil {
				converted, convErr := convertManagedTablePartRows(rows, tp)
				if convErr != nil {
					return nil, convErr
				}
				result[tp.Name] = converted
		...
		for _, row := range rows {
			...
			if !empty {
				...
				for _, f := range tp.Fields {
					// Непришедший реквизит — это пусто, а не «<nil>»: до #1074
					// здесь стоял безусловный fmt.Sprintf("%v", row[f.Name]), и
					// отсутствующее значение превращалось в строку «<nil>»,
					// которая дальше уезжала в базу наравне с настоящими.
					raw := ""
					if v, ok := row[f.Name]; ok && v != nil {
						raw = fmt.Sprintf("%v", v)
					}
					value, convErr := tablePartFieldValue(f, raw)
					if convErr != nil {
						return nil, fmt.Errorf("таблица %q: %w", tp.Name, convErr)
					}
