...
{{else if eq (str $el.Kind) "ПолеКода"}}
  ...
  <div class="form-group{{if elFill $el}} ob-el-fill{{end}}" data-ob-el="{{$el.Name}}"{{with elLayout $el}} style="{{.}}"{{end}}>
    ...
    <textarea name="{{$fn}}" autocomplete="off" class="code-field" rows="12" spellcheck="false"
      ...
      {{if $el.AccessKey}}accesskey="{{$el.AccessKey}}"{{end}}{{if $ro}} readonly{{end}}{{if and $hChg (or (not $ro) (elReadOnlyDynamic $ctx $el))}} data-ob-fire-change="{{$el.Name}}"{{end}}>{{index $ctx.Values $fn}}</textarea>
  ...
  <div class="form-group{{if elFill $el}} ob-el-fill{{end}}" data-ob-el="{{$el.Name}}"{{with elLayout $el}} style="{{.}}"{{end}}>
    ...
    {{if $f}}
      {{if isRef (str $f.Type)}}
        <div class="managed-control-row" style="display:flex;gap:6px;align-items:center">
          ...
          {{/* Атрибут data-ob-fire-change рисуется и у заблокированного поля,
               если запрет условный: клиент снимает запрет по ответу события, а
               повесить обработчик на несуществующий атрибут уже не может —
               каскад «филиал → направление → товарная группа» обрывался на
               первом шаге и оживал только после записи документа. */}}
          ...
          {{if and $ro (index $ctx.Values $fn)}}<input type="hidden" name="{{$fn}}" value="{{index $ctx.Values $fn}}" data-ob-readonly-value="1">{{end}}
      ...
      {{else if isEnum (str $f.Type)}}
        <select name="{{$fn}}"{{if and $req (not $ro)}} required{{end}}{{if $el.AccessKey}} accesskey="{{$el.AccessKey}}"{{end}}{{if $ro}} disabled{{end}}{{if and $hChg (or (not $ro) (elReadOnlyDynamic $ctx $el))}} data-ob-fire-change="{{$el.Name}}"{{end}}>
          <option value="">{{if $ro}}—{{else}}— выбрать —{{end}}</option>
      ...
      {{else if eq (str $f.Type) "date"}}
        <input type="datetime-local" name="{{$fn}}" value="{{index $ctx.Values $fn}}"{{if and $req (not $ro)}} required{{end}}{{if $el.AccessKey}} accesskey="{{$el.AccessKey}}"{{end}}{{if $ro}} readonly{{end}}{{if and $hChg (or (not $ro) (elReadOnlyDynamic $ctx $el))}} data-ob-fire-change="{{$el.Name}}"{{end}}>
      ...
      {{else if eq (str $f.Type) "number"}}
        <input type="text" autocomplete="off" inputmode="decimal" pattern="[+-]?([0-9]+([.,][0-9]+)?|[.,][0-9]+)" name="{{$fn}}" value="{{index $ctx.Values $fn}}" title="Введите число; десятичный разделитель — запятая или точка"{{if and $req (not $ro)}} required{{end}}{{if $el.AccessKey}} accesskey="{{$el.AccessKey}}"{{end}}{{if $ro}} readonly{{end}}{{if and $hChg (or (not $ro) (elReadOnlyDynamic $ctx $el))}} data-ob-fire-change="{{$el.Name}}"{{end}}>
      ...
      {{else if $el.Multiline}}
        <textarea name="{{$fn}}" autocomplete="off" rows="5" style="width:100%"{{if and $req (not $ro)}} required{{end}}{{if $el.AccessKey}} accesskey="{{$el.AccessKey}}"{{end}}{{if $ro}} readonly{{end}}{{if and $hChg (or (not $ro) (elReadOnlyDynamic $ctx $el))}} data-ob-fire-change="{{$el.Name}}"{{end}}>{{index $ctx.Values $fn}}</textarea>
      {{else}}
        <input type="text" autocomplete="off" name="{{$fn}}" value="{{index $ctx.Values $fn}}"{{if and $req (not $ro)}} required{{end}}{{if $el.AccessKey}} accesskey="{{$el.AccessKey}}"{{end}}{{if $ro}} readonly{{end}}{{if $el.Mask}} pattern="{{$el.Mask}}"{{end}}{{if $el.InputMask}} data-ob-input-mask="{{$el.InputMask}}"{{if inputMaskDigitsOnly $el.InputMask}} inputmode="numeric"{{end}}{{end}}{{if $el.Hint}} title="{{$el.Hint}}"{{end}}{{if and $hChg (or (not $ro) (elReadOnlyDynamic $ctx $el))}} data-ob-fire-change="{{$el.Name}}"{{end}}>
      {{end}}
    {{else if eq (str $el.Type) "file"}}
    ...
    {{else}}
      ...
      {{if and $attr (attrRefEntity $attr.TypeRef) (or (index $ctx.RefOptions $fn) $choiceCtx)}}
        ...
        <div class="managed-control-row" style="display:flex;gap:6px;align-items:center">
          <select class="managed-fill-control" id="ref-{{$fn}}" name="{{$fn}}"{{if noChoiceDropdown $el}} tabindex="-1" aria-readonly="true" data-ob-no-dropdown="1"{{end}} style="flex:1{{if noChoiceDropdown $el}};pointer-events:none;background:#fff;-webkit-appearance:none;-moz-appearance:none;appearance:none{{end}}" data-ref-entity="{{attrRefEntity $attr.TypeRef}}"{{if $choiceCtx}} data-ref-choice-context="{{$choiceCtx}}"{{end}}{{if and $req (not $ro)}} required{{end}}{{if $el.AccessKey}} accesskey="{{$el.AccessKey}}"{{end}}{{if $ro}} disabled{{end}}{{if and $hChg (or (not $ro) (elReadOnlyDynamic $ctx $el))}} data-ob-fire-change="{{$el.Name}}"{{end}}>
            <option value="">{{if $ro}}—{{else}}— выбрать —{{end}}</option>
            {{range managedRefOptions $ctx $el $fn}}
            <option value="{{index . "id"}}"{{if index . "_choice_outside_filter"}} data-ob-choice-outside-filter="1"{{end}} {{if eq (index . "id") (index $ctx.Values $fn)}}selected{{end}}>{{index . "_label"}}</option>
            {{end}}
      ...
      {{else}}
        ...
        <input type="text" name="{{$fn}}" value="{{index $ctx.Values $fn}}" placeholder="{{$fn}}" style="background:#fef9c3"{{if and $req (not $ro)}} required{{end}}{{if $el.AccessKey}} accesskey="{{$el.AccessKey}}"{{end}}{{if $ro}} readonly{{end}}
          title="Реквизит формы '{{$el.DataPath}}' не найден среди полей сущности"{{if and $hChg (or (not $ro) (elReadOnlyDynamic $ctx $el))}} data-ob-fire-change="{{$el.Name}}"{{end}}>
      {{end}}
    {{end}}
...
{{else if eq (str $el.Kind) "ПолеСписка"}}
  ...
  <div class="form-group{{if elFill $el}} ob-el-fill{{end}}" data-ob-el="{{$el.Name}}"{{with elLayout $el}} style="{{.}}"{{end}}>
    <label>{{fieldTitleRU $el.TitleMap $fn}}{{if $effectiveReq}} <span style="color:#dc2626">*</span>{{end}}</label>
    <select name="{{$fn}}"{{if and $req (not $ro)}} required{{end}}{{if $el.AccessKey}} accesskey="{{$el.AccessKey}}"{{end}}{{if and (not $ro) (hasHandler $el "НачалоВыбора")}} data-el="{{$el.Name}}" data-ob-list-choice="{{$el.Name}}"{{end}}{{if $ro}} disabled{{end}}{{if and $hChg (or (not $ro) (elReadOnlyDynamic $ctx $el))}} data-ob-fire-change="{{$el.Name}}"{{end}}>
      <option value="">{{if $ro}}—{{else}}— выбрать —{{end}}</option>
...
{{else if eq (str $el.Kind) "Флажок"}}
  ...
  <div class="form-group managed-checkbox" data-ob-el="{{$el.Name}}" style="display:flex;align-items:center;gap:8px;{{elLayout $el}}">
    ...
    <input type="checkbox" id="cb-{{$fn}}" name="{{$fn}}" value="true"{{if $el.AccessKey}} accesskey="{{$el.AccessKey}}"{{end}}
      {{if eq (index $ctx.Values $fn) "true"}}checked{{end}}{{if $ro}} disabled{{end}}{{if and $hChg (or (not $ro) (elReadOnlyDynamic $ctx $el))}} data-ob-fire-change="{{$el.Name}}"{{end}}>
...
{{else if eq (str $el.Kind) "ПолеДаты"}}
  ...
  <div class="form-group{{if elFill $el}} ob-el-fill{{end}}" data-ob-el="{{$el.Name}}"{{with elLayout $el}} style="{{.}}"{{end}}>
    <label>{{fieldTitleRU $el.TitleMap $fn}}{{if $effectiveReq}} <span style="color:#dc2626">*</span>{{end}}</label>
    <input type="date" name="{{$fn}}" value="{{if ge (len $dv) 10}}{{slice $dv 0 10}}{{else}}{{$dv}}{{end}}"{{if and $req (not $ro)}} required{{end}}{{if $el.AccessKey}} accesskey="{{$el.AccessKey}}"{{end}}{{if $ro}} readonly{{end}}{{if and $hChg (or (not $ro) (elReadOnlyDynamic $ctx $el))}} data-ob-fire-change="{{$el.Name}}"{{end}}>
  </div>
  ...
  <div class="form-group{{if elFill $el}} ob-el-fill{{end}}" data-ob-el="{{$el.Name}}"{{with elLayout $el}} style="{{.}}"{{end}}>
    ...
    {{else}}
      <div class="switch-options" style="display:flex;flex-wrap:wrap;gap:12px;padding:4px 0">
        {{if $enum}}
          {{range $i, $opt := index $ctx.EnumOptions $fn}}<label style="display:inline-flex;align-items:center;gap:5px;cursor:pointer"><input type="radio" name="{{$fn}}" value="{{$opt.Value}}"{{if and $req (not $ro)}} required{{end}}{{if and (eq $i 0) $el.AccessKey}} accesskey="{{$el.AccessKey}}"{{end}}{{if eq $opt.Value $cur}} checked{{end}}{{if $ro}} disabled{{end}}{{if and $hChg (or (not $ro) (elReadOnlyDynamic $ctx $el))}} data-ob-fire-change="{{$el.Name}}"{{end}}> {{$opt.Label}}</label>{{end}}
        {{else}}
          {{range $i, $opt := $el.Options}}<label style="display:inline-flex;align-items:center;gap:5px;cursor:pointer"><input type="radio" name="{{$fn}}" value="{{$opt.ValueStr}}"{{if and $req (not $ro)}} required{{end}}{{if and (eq $i 0) $el.AccessKey}} accesskey="{{$el.AccessKey}}"{{end}}{{if eq $opt.ValueStr $cur}} checked{{end}}{{if $ro}} disabled{{end}}{{if and $hChg (or (not $ro) (elReadOnlyDynamic $ctx $el))}} data-ob-fire-change="{{$el.Name}}"{{end}}> {{$opt.Label}}</label>{{end}}
        {{end}}
      </div>
    {{end}}
    {{if $el.Hint}}<small style="color:#94a3b8;font-size:11px">{{$el.Hint}}</small>{{end}}
