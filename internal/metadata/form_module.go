...
type FormElement struct {
	...
	// (аналог ВыборГруппИЭлементов в 1С). Подбор ссылочного поля всегда
	// исключает папки, и для справочника, где значение по смыслу является
	// группой, поле оказывается невыбираемым: список пуст, а уже записанное
	// значение платформа считает недопустимым, чистит его и помечает форму
	// изменённой. Так ведёт себя населённый пункт в адресном классификаторе:
	// у города есть улицы, поэтому он группа.
	ChoiceFolders bool `yaml:"choice_folders,omitempty"`
	// Primary — кнопка основного действия формы: рисуется акцентным стилем
	// (btn-primary), как платформенная «ОК». Нужен формам, где штатный ряд
	// «Записать / ОК / Закрыть» скрыт и единственное действие объявлено
	// своей кнопкой: без акцента она теряется среди вспомогательных.
	// Фоном группы это не заменить — рамку группы отключить нечем.
	...
	// ChoiceDropdown — показывать ли варианты прямо в выпадающем списке
	// ссылочного поля. По умолчанию да: <select> получает предзагруженную
	// страницу вариантов, и мелкий справочник выбирается одним кликом.
	//
	// `choice_dropdown: false` оставляет в списке только плейсхолдер и текущее
	// значение, а выбор уходит в форму подбора («…»). Это нужно там, где
	// предзагруженная страница вводит в заблуждение: у большого справочника она
	// показывает произвольные первые N записей, и оператор выбирает из них, не
	// подозревая, что видит не весь список. Значение поля при этом продолжает
	// отображаться — иначе заполненное поле выглядело бы пустым.
	ChoiceDropdown *bool `yaml:"choice_dropdown,omitempty"`
	// EditableAdminOnly — поле редактирует только администратор, остальным оно
	// нередактируемо всегда. В отличие от readonly_when, условие тут не от
	// данных записи, а от того, кто смотрит, и вычисляется на сервере при
	// отрисовке: реквизит формы с признаком админа заполняется клиентским
	// ПриОткрытии, то есть уже после рендера, и поле успевало мигнуть
	// редактируемым.
	EditableAdminOnly bool `yaml:"editable_admin_only,omitempty"`
	// RefCardButton — показывать ли у ЭТОГО ссылочного поля кнопку «Открыть
	// карточку» (🔍). Переопределяет одноимённый ключ формы: у формы он
	// выключает кнопку всюду, а нужно бывает точечно — у полей, где карточка
	// справочника оператору не нужна и лишняя кнопка только мешает.
...
type FormChoiceCondition struct {
	...
	// Ref — литеральная ссылка на конкретный элемент справочника вместо
	// значения с формы. Нужна для условий, где сравнивать не с чем: «исключить
	// вот эту ветку» не зависит от того, что выбрал пользователь. Реквизит
	// формы тут не годится — при первой отрисовке его значения ещё нет, и
	// подбор честно выдаёт пустоту.
...
// InputMaskHint — подсказка пустого поля по маске ввода: заполнители
// заменяются подчёркиванием, литералы остаются на местах. «(000)000-00-00»
// превращается в «(___)___-__-__», «Пл0000000» — в «Пл_______».
//
// Нужна потому, что маска до первого символа ничем себя не выдаёт: поле
// выглядит обычным, и человек не знает ни формата, ни того, что «Пл»
// подставится само.
func InputMaskHint(mask string) string {
	if mask == "" {
		return ""
	}
...
type FormModule struct {
	...
	// ключом на форму.
	RefCardButton *bool `yaml:"ref_card_button,omitempty"`
	// RefCardButtonAdminOnly — кнопку «Открыть карточку» видит только
	// администратор. Оператору справочники вести не положено, и лупа рядом с
	// каждым значением мешает читать плотную форму, а администратору она нужна
	// именно на форме: отсюда видно, какой элемент выбран.
	//
