...
func TestJoinedRefFieldMatrix(t *testing.T) {
	dbtest.ForEachDialect(t, func(t *testing.T, db *storage.DB) {
		...
		for _, c := range []struct{ name, q, want, wantErr string }{
			{
			...
			{
				"навигация через точку у присоединённого в СГРУППИРОВАТЬ ПО",
				`ВЫБРАТЬ П.ГлавныйУзел.Наименование ИЗ Справочник.Базы КАК Б ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Планы КАК П ПО П.Код = Б.Код СГРУППИРОВАТЬ ПО П.ГлавныйУзел.Наименование`,
				"Главная",
				"",
			},
			{
				"навигация через точку у присоединённого в УПОРЯДОЧИТЬ ПО",
				`ВЫБРАТЬ П.Наименование ИЗ Справочник.Базы КАК Б ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Планы КАК П ПО П.Код = Б.Код УПОРЯДОЧИТЬ ПО П.ГлавныйУзел.Наименование`,
				"План-1",
				"",
			},
			{
				// Навигация внутри СОБСТВЕННОГО ПО запрещена явно: авто-JOIN
				// для П.ГлавныйУзел пришлось бы поставить раньше самого П.
				// Поддержка потребует менять порядок JOIN — отдельная работа.
				"навигация через точку у присоединённого в собственном ПО",
				`ВЫБРАТЬ П.Наименование ИЗ Справочник.Базы КАК Б ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Планы КАК П ПО П.ГлавныйУзел.Наименование = Б.Наименование`,
				"",
				"навигация по ссылке присоединённого источника внутри ПО не поддерживается",
			},
			{
				// А в ПО СЛЕДУЮЩЕГО соединения тот же авто-JOIN уже объявлен
				// выше по тексту и работает — это исправление второго круга.
				"навигация через точку у присоединённого в ПО следующего соединения",
				`ВЫБРАТЬ Б2.Наименование ИЗ Справочник.Базы КАК Б ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Планы КАК П ПО П.ГлавныйУзел = Б.Ссылка ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Базы КАК Б2 ПО Б2.Наименование = П.ГлавныйУзел.Наименование`,
				"Главная",
				"",
			},
			{
			...
			t.Run(c.name, func(t *testing.T) {
				...
				if c.wantErr != "" {
					if err == nil {
						t.Fatalf("ожидали ошибку %q, получили SQL: %s", c.wantErr, r.SQL)
					}
